...
static void render_callback(Canvas* const canvas, void* ctx) {
    ...
    elements_multiline_text_aligned(canvas, 64, 2, AlignCenter, AlignTop, "LaserModule");

    canvas_set_font(canvas, FontSecondary);

    if(!plugin_state->is_on) {
        elements_multiline_text_aligned(
            canvas, 64, 28, AlignCenter, AlignTop, "Press OK button to Fire Laser!");
    } else {
        elements_multiline_text_aligned(canvas, 64, 28, AlignCenter, AlignTop, "FiRiNg MaH LaSeR!!");
        elements_multiline_text_aligned(
            canvas, 64, 40, AlignCenter, AlignTop, "Press OK button to Deactivate Laser!");
    }
